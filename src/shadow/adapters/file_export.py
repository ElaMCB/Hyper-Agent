...
def _first_non_blank(r: dict, *keys: str) -> Any:
    for key in keys:
        if key not in r:
            continue
        value = r.get(key)
        ...
        if isinstance(r, dict):
            out.append(
                TeamMember(
                    ...
                    on_vacation=_parse_bool(_first_non_blank(r, "on_vacation", "vacation")),
...
def load_allocations_from_json(path: Path) -> list[CapacityAllocation]:
    ...
    for r in raw:
        if isinstance(r, dict):
            focus = _parse_int(_first_non_blank(r, "focus_pct", "pct", "allocation_pct"))
            out.append(
                CapacityAllocation(
                    id=str(r.get("id", r.get("key", ""))),
