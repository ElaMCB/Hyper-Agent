...
def _first_nonempty(r: dict, *keys: str) -> Any:
    for key in keys:
        if key in r:
            value = r.get(key)
            if value is None:
        ...
        if isinstance(r, dict):
            out.append(
                TeamMember(
                    ...
                    on_vacation=_parse_bool(_first_nonempty(r, "on_vacation", "vacation")),
...
def load_allocations_from_json(path: Path) -> list[CapacityAllocation]:
    ...
    for r in raw:
        if isinstance(r, dict):
            pct = _first_nonempty(r, "focus_pct", "pct", "allocation_pct")
            try:
